...
import comparadores.OrdenamentoNomeClienteDecrescente;
...
public class TesteItemTresParteCinco {
	public static void main(String...args) {
		/**
		 * @author Bruno Cipriano
		 * O objetivo desse Teste é verificar a implementação da interface "Comparator"
		 * para ordenar os contratos de servicos em ordem alfabetica DECRESCENETE com base 
		 ...
		 * ordena os contratos de serviço em ordem alfabetica decrescente dos nomes dos clientes de cada  
		 * contrato de serviço.
		 */
		ArrayList<Contrato> listaContratosDeServicosPrestados = historico.listarContratosDeServico(new OrdenamentoNomeClienteDecrescente());

		
		//Verificação do processo de ordenamento:
		for(Contrato contrato : listaContratosDeServicosPrestados) {
			System.out.println("Nome: "+contrato.getCliente().getNome()+" Servico: "+contrato.getServico().getDescricao());
		}
